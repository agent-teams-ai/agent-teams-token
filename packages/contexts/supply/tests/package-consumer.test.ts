...
test("black-box consumer can use only the declared genesis-manifest subpath", async (context) => {
  const packageRoot = process.cwd().endsWith("/packages/contexts/supply") ? process.cwd() : resolve(process.cwd(), "packages/contexts/supply"), repositoryRoot = resolve(packageRoot, "../../.."), consumer = join(repositoryRoot, ".local/package-consumer"), scope = join(consumer, "node_modules/@agent-teams");
...
async function exitCode(entry: string): Promise<number | null> { return new Promise((resolveCode, reject) => { const child = spawn(process.execPath, [entry], { stdio: "ignore" }); child.on("error", reject); child.on("close", resolveCode); }); }
