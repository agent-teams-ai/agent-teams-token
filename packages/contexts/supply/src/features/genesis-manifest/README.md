...
The package exports only `@agent-teams/supply/genesis-manifest`. Its public subpath deliberately omits the raw compiler and its ports; local compilation remains an internal composition flow that strictly parses source text before invoking the typed compiler. Successful compiler output is content-addressed under the ignored `.local/genesis/` directory and is usable only when its adjacent `READY` marker matches both source and artifact digests.
