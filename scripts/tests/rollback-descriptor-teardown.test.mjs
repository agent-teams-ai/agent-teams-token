...
function injectedUncertainClose(failureIndex) {
  const calls = [];
  ...
  const restore = setDescriptorCloseImplementationForTest((descriptor) => {
    ...
    if (current === failureIndex) {
      ...
      do {
        ...
        if (reusedDescriptor < descriptor) {lowerDescriptors.push(reusedDescriptor);}
      } while (reusedDescriptor < descriptor);
      for (const lower of lowerDescriptors) {closeSync(lower);}
...
function assertOtherDescriptorsClosed(descriptors, reusedDescriptor) {
