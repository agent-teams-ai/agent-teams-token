...
test("root check delegates to the authenticated pnpm bootstrap boundary", (context) => {
  const fixture = createRootFixture(context);
  const result = runRoot(fixture, ["check", "--sentinel"]);
  assert.equal(result.status, 0, result.stderr);
  assert.deepEqual(readFileSync(fixture.log, "utf8").trim().split("\n"), [
    "bootstrap:run-pnpm check --sentinel",
  ]);
  assert.equal(existsSync(fixture.marker), false);
});

test("every toolchain-mediated root command stops at its authenticated bootstrap boundary", (context) => {
  const fixture = createRootFixture(context);
  const cases = [
    [["doctor", "--scope=core"], "bootstrap:doctor --scope=core"],
    [["check"], "bootstrap:run-pnpm check"],
    [["solana"], "bootstrap:run-solana"],
  ];
...
test("doctor wiring verifies and executes through the archive-derived Node", () => {
  ...
  assert.match(bootstrap, /^export PATH=\/usr\/bin:\/bin$/mu);
});
