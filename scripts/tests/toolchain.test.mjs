...
test("Bash bootstrap starts from checksum-pinned Node without a system Node fallback", () => {
  ...
  assert.match(bootstrap, /exec 8<"\$token_snapshot_path"/u);
  assert.doesNotMatch(bootstrap, /exec 9</u);
  assert.match(bootstrap, /token_descriptor_fingerprint\(\)/u);
  assert.match(bootstrap, /%d:%i:%p:%u:%g:%z:%m:%c/u);
  assert.match(bootstrap, /%d:%i:%f:%u:%g:%s:%y:%z/u);
  ...
  assert.match(bootstrap, /token_snapshot_sha256=\$\(token_sha256 \/dev\/fd\/8\)/u);
  assert.match(bootstrap, /"\$token_node_tar_flag" \/dev\/fd\/8/u);
...
test("tampered pnpm payload and wrapper are rejected and restored from verified cache", (context) => {
  ...
  assert.match(nodeWrapperText, /GIT_CONFIG_COUNT=4/u);
  assert.match(nodeWrapperText, /GIT_CONFIG_KEY_0=core\.fsmonitor GIT_CONFIG_VALUE_0=false/u);
  assert.match(nodeWrapperText, /GIT_CONFIG_KEY_1=credential\.helper GIT_CONFIG_VALUE_1=/u);
  assert.match(nodeWrapperText, /GIT_CONFIG_KEY_3=safe\.directory/u);
  assert.doesNotMatch(nodeWrapperText, /NODE_OPTIONS|NODE_PATH|HTTP_PROXY|npm_config/iu);
