...
test("Bash bootstrap starts from checksum-pinned Node without a system Node fallback", () => {
  ...
  assert.match(bootstrap, /token_descriptor_identity\(\)/u);
  assert.match(bootstrap, /\/usr\/bin\/stat -f '%d:%i' <&"\$token_descriptor"/u);
  assert.match(bootstrap, /\/usr\/bin\/stat -L -c '%d:%i' "\/proc\/self\/fd\/\$token_descriptor"/u);
  assert.match(bootstrap, /token_path_identity\(\)/u);
  assert.equal([...bootstrap.matchAll(/token_descriptor_identity [789]/gu)].length, 6);
