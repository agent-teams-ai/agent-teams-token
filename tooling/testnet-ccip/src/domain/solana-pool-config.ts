...
export const POOL_CONFIG_OPERATIONS = ["init-chain-remote-config", "append-remote-pool-addresses", "set-chain-rate-limit", "create-lookup-table", "set-pool"] as const;
...
function validate(e: SolanaPoolConfigExpectation): void {
  ...
  if (e.operation === "create-lookup-table" || e.operation === "set-pool") {
...
export function poolConfigInstructions(e: SolanaPoolConfigExpectation): MintInstruction[] {
  ...
  const data = init ? Buffer.concat([u32(0), u32(32), remoteBytes(REMOTE_TOKEN), Buffer.from([9])]) :
    Buffer.concat([u32(1), u32(32), remoteBytes(REMOTE_POOL)]);
  return [instruction(BURNMINT_PROGRAM, accounts, Buffer.concat([discriminator(init ? "init_chain_remote_config" : "append_remote_pool_addresses"), ...prefix, data]))];
}
