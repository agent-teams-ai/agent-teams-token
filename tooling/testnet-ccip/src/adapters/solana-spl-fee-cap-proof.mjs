...
/** Encode only the four classic SPL Token instructions. Account identities are
 * checked by buildUnsignedSplFeeCapProof before this is used in a packet. */
export function splFeeCapInstructions(input) {
  const sourceAmount = decimal(input?.sourceAmount, 'AGTMAI amount');
  const cap = decimal(input?.cap, 'fee cap');
  if (!Number.isInteger(input?.feeDecimals) || input.feeDecimals < 0 || input.feeDecimals > 18) {
    fail('fee decimals outside supported range');
  }
  const approve = (tokenAta, mint, amount, decimals) => instruction(SPL_TOKEN_PROGRAM,
    [meta(tokenAta, true), meta(mint), meta(input.billingPda), meta(input.payer, false, true)],
  ...
  return [approve(input.sourceAta, input.sourceMint, sourceAmount, 9),
    approve(input.feeAta, input.feeMint, cap, input.feeDecimals),
    revoke(input.feeAta), revoke(input.sourceAta)];
}

/** EVM receiver in a CCIP SVM message is a left-zero-padded 20-byte address. */
export function assertAbi32EvmReceiver(receiver) {
  if (!Buffer.isBuffer(receiver) || receiver.length !== 32 ||
      !receiver.subarray(0, 12).equals(Buffer.alloc(12)) ||
      receiver.subarray(12).equals(Buffer.alloc(20))) {fail('receiver must be a nonzero ABI32 EVM address');}
}
...
function mintState(PublicKey, raw, { address, decimals, minimumSupply, label }) {
  ...
  if (bytes.readUInt32LE(0) > 1 || bytes.readUInt32LE(0) === 1 && bytes.subarray(4, 36).equals(Buffer.alloc(32)) ||
      bytes[45] !== 1 || bytes[44] !== decimals || bytes.readUInt32LE(46) > 1 ||
...
function validateCapturedBalances(PublicKey, input, sourceAmount, cap) {
  ...
  mintState(PublicKey, input.state?.sourceMint,
    { address: input.sourceMint, decimals: 9, minimumSupply: sourceAmount, label: 'AGTMAI mint' });
  mintState(PublicKey, input.state?.feeMint,
    { address: input.feeMint, decimals: input.feeDecimals, minimumSupply: cap, label: 'fee mint' });
  tokenState(PublicKey, input.state?.sourceAta,
    { address: input.sourceAta, mint: input.sourceMint, owner: input.payer, amount: sourceAmount, label: 'AGTMAI ATA' });
  tokenState(PublicKey, input.state?.feeAta,
    { address: input.feeAta, mint: input.feeMint, owner: input.payer, amount: cap, label: 'fee ATA' });
  tokenState(PublicKey, input.state?.feeReceiverAta,
    { address: input.feeReceiverAta, mint: input.feeMint, owner: billingPda, amount: 0n, label: 'fee receiver ATA' });
  return { billingPda, routerConfig };
}
function validateFrozenSend(PublicKey, input, sourceAmount, { billingPda, routerConfig }) {
  ...
  const selectorBytes = u64(ETHEREUM_SELECTOR);
  const destChain = PublicKey.findProgramAddressSync([Buffer.from('dest_chain_state'), selectorBytes],
    key(PublicKey, ROUTER_PROGRAM, 'Router'))[0].toBase58();
  const nonce = PublicKey.findProgramAddressSync([Buffer.from('nonce'), selectorBytes,
    key(PublicKey, input.payer, 'payer').toBuffer()], key(PublicKey, ROUTER_PROGRAM, 'Router'))[0].toBase58();
  const anchors = new Map([[0, meta(routerConfig)], [1, meta(destChain, true)], [2, meta(nonce, true)],
    [3, meta(input.payer, true, true)], [4, meta(SYSTEM_PROGRAM)],
    ...
    [13, meta(feeTokenConfig)], [18, meta(input.sourceAta, true)], [28, meta(input.sourceMint, true)]]);
  ...
  // The retained reverse instruction profile uses these writable positions
  // for its one-token BurnMint route. A separately pinned SDK capture is still
  // required before this can qualify a send candidate.
  const writable = new Set([1, 2, 3, 7, 8, 18, 20, 24, 25, 28]);
  for (const [index, account] of send.accounts.entries()) {
    key(PublicKey, account?.address, 'ccip_send account');
    if (account.isWritable !== writable.has(index) || account.isSigner !== (index === 3)) {
      fail(`invalid ccip_send account ${index} privileges`);
    }
  }
  return send;
}
...
export function buildUnsignedSplFeeCapProof(provider, input) {
  ...
  validatePrograms(input);
  const sourceAmount = decimal(input.sourceAmount, 'AGTMAI amount');
  const cap = decimal(input.cap, 'fee cap');
  validateSnapshot(input);
  const balances = validateCapturedBalances(PublicKey, input, sourceAmount, cap);
  const send = validateFrozenSend(PublicKey, input, sourceAmount, balances);
  const [approveSource, approveFee, revokeFee, revokeSource] = splFeeCapInstructions(input);
  const instructions = [approveSource, approveFee, structuredClone(send), revokeFee, revokeSource];
