...
import type { PoolInitObservation, PoolStateEvidence, SignedPoolInitTransaction } from "../application/solana-pool-init-journal.ts";
...
export function createSolanaPoolInitRpc(
  ...
  async function absent(signed: SignedPoolInitTransaction, signatureStatus: unknown): Promise<PoolInitObservation> {
  ...
  async function readPool(intent: SolanaPoolInitEnvelope, slot: number): Promise<PoolStateEvidence> {
    const response = object(await rpc("getAccountInfo", [intent.pool,
      { encoding: "base64", commitment: "finalized", minContextSlot: slot }]));
    const account = object(response.value);
    if (integer(object(response.context).slot) < slot || account.owner !== BURNMINT_PROGRAM ||
      account.executable !== false || !Array.isArray(account.data) || account.data[1] !== "base64") {
      throw new Error("Invalid finalized pool account");
    }
    return verifyState(encoded(account.data[0]), intent);
  }
  async function observe(signed: SignedPoolInitTransaction, intent: SolanaPoolInitEnvelope): Promise<PoolInitObservation> {
    try {
      await chain();
      ...
      const poolState = meta.err === null ? await readPool(intent, slot) : null;
      ...
      return { kind: "finalized", signature: signed.signature, messageBase64, slot: String(slot),
        err: meta.err === null ? null : { transactionError: meta.err }, poolState };
