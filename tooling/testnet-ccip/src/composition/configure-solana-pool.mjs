...
export async function verifyPoolConfigPredecessor(settings, expected, { sdk, rpc, registrationSdk, registrationRpc }) {
  ...
  const provider = registration ? registrationSdk : sdk, observer = registration ? registrationRpc : rpc;
  const previous = provider.derive(record.intent);
  if (expected.operation === "set-pool" && (previous.alt !== expected.alt || previous.recentSlot !== expected.recentSlot)) { throw new Error("Set pool must use the finalized predecessor ALT"); }
