...
export function parseProductionObservation(input: unknown): ProductionObservation {
  const value = record(input);
  keys(value, ["chainId", "sender", "pendingNonce", "blockNumber", "blockHash", "observedAt", "expiresAt", "operations", "checkedAddresses", "occupiedAddresses", "authority"], ["chainId", "sender", "pendingNonce", "blockNumber", "blockHash", "observedAt", "expiresAt", "operations", "checkedAddresses", "occupiedAddresses", "authority"]);
  ...
  mapArray(value.operations, operation => {
    const item = record(operation); keys(item, ["id", "address", "nestedAddress", "creation", "runtime", "calldata", "value", "nonce"]);
    if (!stringValue(item.id)) {reject("VALUE");} optional(item.address, address); optional(item.nestedAddress, address); optional(item.creation, bytes); optional(item.runtime, bytes); optional(item.calldata, bytes); optional(item.value, decimal); optional(item.nonce, decimal); return item;
  });
