...
export function independentlyApproveRawArtifact(
  ...
  const constructorArguments = encodeConstructor(
    parseConstructor(parsed.fixture),
  );
 ...
 */
function portableCompilerInputSha256(
...
function canonicalBuildInfoSha256(
  build: Record<string, unknown>,
): `0x${string}` {
...
function encodeConstructor(value: ConstructorValues): `0x${string}` {
