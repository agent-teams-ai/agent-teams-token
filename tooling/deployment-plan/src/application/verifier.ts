...
export async function independentlyVerifyRpc(request: RpcVerificationRequest): Promise<void> {
  ...
  if (quantity(history.oldestBlock).toString() !== quote.observation.feeHistoryNewestBlock) {
    fail("RPC_FEE_HISTORY_CHANGED", "fee-history head changed");
  }
...
function validateQuoteFreshness(
  ...
  if (
    ...
    || nowSeconds - blockTime > parseUint(roots.quoteTtlSeconds, "ttl")
  ) {
    fail("STALE_QUOTE", "quote is stale at the exact expiry boundary");
  }
}
...
function validateRpcBlocks(
  ...
  if (
    ...
    fail("RPC_BLOCK_CHANGED", "RPC block binding changed or reorged");
  }
}
