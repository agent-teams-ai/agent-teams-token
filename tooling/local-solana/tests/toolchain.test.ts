...
async function fixture(): Promise<{ readonly root: string; readonly binaries: Record<string, string>; readonly command: FakeCommand }> {
  ...
  await writeFile(join(root, "tooling/toolchain.lock.json"), JSON.stringify({ tools: { agave: { version: "4.2.1", splTokenVersion: "5.6.1", platforms: { [platform]: artifact } } } }));
  return { root, binaries, command: new FakeCommand() };
}
...
test("pinned resolver ignores hostile PATH and accepts warm verified binaries", async () => {
  ...
  try { const paths = await new PinnedToolResolver(value.root, value.command).resolve(); assert.equal(paths.splToken, value.binaries["spl-token"]); assert.equal(value.command.calls.every((path) => path.startsWith(value.root)), true); }
