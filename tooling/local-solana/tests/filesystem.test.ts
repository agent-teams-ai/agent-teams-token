...
import type { RunPaths, ValidatorIdentity } from "../src/application/ports.ts";
...
test("real parent SIGKILL reclaim authenticates, terminates and awaits its validator child", { skip: process.platform === "linux" || process.platform === "darwin" ? false : "authenticated stale-child reclaim requires Linux procfs or native Darwin ps identity" }, async () => {
  ...
  let validator: ValidatorIdentity | undefined; let token = "";
  try {
    ...
    assert.ok(ready, `validator helper did not become ready: ${diagnostics}`);
    const lease = JSON.parse(await readFile(join(ready.runDirectory, ".agtmai-local-solana-lease.json"), "utf8")); validator = lease.validator; token = lease.token;
    parent.kill("SIGKILL"); await new Promise<void>((resolve) => { parent.once("close", () => resolve()); });
    ...
    for (const child of [parent, neighbour]) {
      if (child.exitCode === null && child.signalCode === null) { const closed = once(child, "close"); child.kill("SIGKILL"); await closed; }
    }
    if (validator === undefined) {
      // A helper failure may happen after registration but before ready publication.
      for (const name of await readdir(runRoot).catch((cause) => { if (cause.code === "ENOENT") { return []; } throw cause; })) {
        const lease = JSON.parse(await readFile(join(runRoot, name, ".agtmai-local-solana-lease.json"), "utf8"));
        if (lease.validator !== null) { validator = lease.validator; token = lease.token; }
      }
    }
    if (validator !== undefined) { await stopFixtureValidator(validator, token); }
...
async function stopFixtureValidator(identity: ValidatorIdentity, token: string): Promise<void> {
  const exited = async (): Promise<boolean> => {
    ...
    try {
      assert.equal(await processStartIdentity(identity.pid), identity.startTime, "fixture PID was replaced");
      if (process.platform !== "linux") { return false; }
      const value = await readFile(`/proc/${identity.pid}/stat`, "utf8"); const fields = value.slice(value.lastIndexOf(")") + 2).trim().split(/\s+/u);
      assert.equal(`linux:${fields[19]}`, identity.startTime);
      return fields[0] === "Z";
    } catch (cause) {
      if (["ENOENT", "ESRCH"].includes((cause as NodeJS.ErrnoException).code ?? "")) {
        try { process.kill(identity.pid, 0); }
        catch (probeCause) { if ((probeCause as NodeJS.ErrnoException).code === "ESRCH") { return true; } }
      }
      throw cause;
    }
  };
  if (await exited()) { return; }
  if (!await authenticateValidatorIdentity(identity, token)) { assert.equal(await exited(), true, "surviving fixture child must authenticate before cleanup"); return; }
  try { process.kill(identity.pid, "SIGKILL"); }
  catch (cause) { if ((cause as NodeJS.ErrnoException).code !== "ESRCH" || !await exited()) { throw cause; } }
  const deadline = Date.now() + 5_000;
  while (!await exited()) { assert.ok(Date.now() < deadline, "fixture child did not exit"); await new Promise((resolve) => { setTimeout(resolve, 50); }); }
}
