...
test("negative authority transactions contain two full Ed25519 signatures", async () => {
  ...
  try {
    ...
    for (const bytes of [await signedRestoreFreezeTransaction(rpc, "http://127.0.0.1:8899/", payerPath, authorityPath, mint, freeze), await signedFreezeAccountTransaction(rpc, "http://127.0.0.1:8899/", payerPath, authorityPath, mint, mint, freeze)]) {
