...
const kinds: readonly TransactionFact["kind"][] = ["create", "revokeFreeze", "mint", "burn", "restoreFreezeAttempt", "freezeAttempt"];
function fixture(): FixtureObservations {
  ...
  return {
    ...
    mintAddress: address, mintAuthority: address, ownerAddress: address, tokenAccountAddress: address,
    ...
    transactions: kinds.map((kind, index) => ({ kind, signature: "2".repeat(64), slot: String(index + 1), confirmationStatus: "finalized", err: index >= 4 ? { InstructionError: [0, "Custom"] } : null, programIds: [CLASSIC_TOKEN_PROGRAM], instructionKinds: [kind], amountBaseUnits: kind === "mint" || kind === "burn" ? amount : null, genesisHash: address })),
  };
}
...
test("verifier fails closed on every core lifecycle invariant", () => {
  const mutations: readonly ((value: FixtureObservations) => FixtureObservations)[] = [
    ...
    (v) => ({ ...v, transactions: v.transactions.map((fact, index) => index === 4 ? { ...fact, err: null } : fact) }),
    (v) => ({ ...v, transactions: v.transactions.map((fact, index) => index === 5 ? { ...fact, programIds: [] } : fact) }),
    (v) => ({ ...v, transactions: v.transactions.map((fact, index) => index === 2 ? { ...fact, amountBaseUnits: "1" } : fact) }),
  ];
