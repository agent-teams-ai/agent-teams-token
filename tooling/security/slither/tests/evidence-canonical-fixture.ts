...
export async function writeCanonicalFixture(
  ...
  const triage = `${JSON.stringify({ schemaVersion: 1, findings: input.findings.map((finding) => ({ schemaVersion: 1, fingerprint: findingFingerprint({ ...finding, location: { ...finding.location, sourceHash: `sha256:${sourceHash}` } }), owner: "fixture-security", disposition: "accepted-design", rationale: "Independently reviewed golden evidence fixture finding.", reviewedAt: "2026-08-29T00:00:00.000Z" })) }, null, 2)}\n`;
