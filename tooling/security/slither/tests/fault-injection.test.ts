...
test("late publication failure leaves no partial bundle and permits a READY-last failure envelope", async () => {
  ...
  try {
    ...
    await assert.rejects(writeReadyEvidence({ output, candidateSha: "a".repeat(40), manifest: hashes.manifest, input, decision, hashes: { config: hashes.config, policy: hashes.policy }, triageHash: hashes.triage, schemaDirectory, canonicalDirectory, assertReadyPrecondition: async () => {throw new Error("injected-final-publication-failure");}, publication: testPublication() }), /injected-final-publication-failure/u);
    await assert.rejects(readFile(output));
    const failure = classifyGateFailure("UNEXPECTED_ENVIRONMENT_FAILURE");
    await writeEnvironmentFailure({ output, candidateSha: "a".repeat(40), stage: failure.stage, errorCode: "UNEXPECTED_ENVIRONMENT_FAILURE", schemaDirectory, assertReadyPrecondition: async () => {}, publication: testPublication() });
