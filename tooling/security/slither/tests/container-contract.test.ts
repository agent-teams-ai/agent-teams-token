...
test("container contract is digest-pinned and hardened", () => {
  ...
  for (const required of [IMAGE, "--name agtmai-slither-test", "--platform linux/amd64", "--network none", "--read-only", "--user 1000:1000", "no-new-privileges=true", "--cap-drop ALL", "--pids-limit 128", "--memory 2g", "--cpus 2", "dst=/input,readonly", "dst=/tools/forge,readonly", "dst=/tools/solc,readonly", "--entrypoint /usr/bin/env", " -i HOME=", "slither . --fail-on pedantic --foundry-ignore-compile", "--skip test --skip script", "PYTHONPATH=/official/python", "CapEff", "pids.max", "memory.max", "cpu.max"]) {assert.match(joined, new RegExp(required.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")));}
