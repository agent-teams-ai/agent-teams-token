...
test("evidence is sanitised, exact-SHA-bound and READY-last", async () => {
  ...
  try {
    await writeReadyEvidence(output, "a".repeat(40), manifest, input, decision, { config: "b".repeat(64), policy: "c".repeat(64) });
...
test("evidence writers reject stale or redirected output paths", async () => {
  ...
  try {
    ...
    await assert.rejects(writeReadyEvidence(output, "a".repeat(40), manifest, input, decision, { config: "b".repeat(64), policy: "c".repeat(64) }));
  } finally { await rm(parent, { recursive: true, force: true }); }
});
...
test("malformed analysis output has a distinct READY-last exit-40 envelope", async () => {
  ...
  try {
    await writeFailureEvidence(output, "e".repeat(40), "output-failure", 40, "analysis", "MALFORMED_JSON");
