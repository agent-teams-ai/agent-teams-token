...
export function makeCompilerEvidence(source: string, sourcePath = "src/A.sol", creationBytecode = "0x6000"): {readonly compilerEvidence: AnalysisInput["compilerEvidence"]; readonly fixtureProof: AnalysisInput["fixtureProof"]; readonly vulnerableFixture: GateManifest["vulnerableFixture"]; readonly fixtureSource: string} {
  ...
  const fixture = build(fixtureSource, "src/Vulnerable.sol", "Vulnerable", creationBytecode, true);
...
function build(source:string,path:string,contract:string,creationBytecode:string,isolated=false):{evidence:AnalysisInput["compilerEvidence"]}{
  const settings={...(isolated ? {} : {remappings:["@openzeppelin/contracts/=lib/openzeppelin-contracts/contracts/","openzeppelin-contracts/=lib/openzeppelin-contracts/contracts/"]}),evmVersion:"paris",optimizer:{enabled:true,runs:200},metadata:{bytecodeHash:"ipfs",appendCBOR:true,useLiteralContent:false},viaIR:false,experimental:false,libraries:{}};
  const metadata={compiler:{version:"0.8.36+commit.8a079791"},settings:{compilationTarget:{[path]:contract},evmVersion:"paris",libraries:{},metadata:{bytecodeHash:"ipfs"},optimizer:{enabled:true,runs:200},remappings:isolated?[]:settings.remappings},sources:{[path]:{}},output:{abi:[]}}; const rawMetadata=JSON.stringify(metadata);
  const input={...(isolated?{version:"0.8.36",language:"Solidity",allowPaths:["/work/contracts/evm","/work/contracts/evm/lib"],basePath:"/work/contracts/evm",includePaths:["/work/contracts/evm"]}:{}),sources:{[path]:{content:source}},settings}; const artifact={abi:[],bytecode:{object:creationBytecode},metadata,rawMetadata}; const buildInfo={solcVersion:"0.8.36",solcLongVersion:"0.8.36",input,output:{contracts:{[path]:{[contract]:{abi:[],metadata:rawMetadata,evm:{bytecode:{object:creationBytecode}}}}}}};
