...
async function makeBundle(output: string): Promise<void> {
  await writeReadyEvidence({ output, candidateSha: sha, manifest, input, decision, hashes: { config: "b".repeat(64), policy: "c".repeat(64) }, triageHash: "d".repeat(64), schemaDirectory, assertReadyPrecondition: async () => {} });
}
