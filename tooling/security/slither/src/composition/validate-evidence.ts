...
import { validateEvidenceEnvironment } from "../adapters/validated-environment.ts";
import { SlitherGateError } from "../domain/model.ts";
try {
  const {repositoryRoot,candidateSha,output}=await validateEvidenceEnvironment(process.env.SLITHER_REPOSITORY_ROOT??process.cwd(),[process.env.GITHUB_SHA,process.env.SLITHER_CANDIDATE_SHA],process.env.SLITHER_EVIDENCE_DIRECTORY);
