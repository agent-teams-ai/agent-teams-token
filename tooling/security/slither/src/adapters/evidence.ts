...
export async function writeReadyEvidence(output: string, candidateSha: string, manifest: GateManifest, input: AnalysisInput, decision: PolicyDecision, hashes: { config: string; policy: string }): Promise<void> {
  ...
  const evidence = {
    ...
    inputs: { closureHash: `sha256:${sha256(JSON.stringify([...manifest.sources, ...manifest.config]))}`, configHash: `sha256:${hashes.config}`, policyHash: `sha256:${hashes.policy}` },
...
export async function writeEnvironmentFailure(output: string, candidateSha: string, stage: string, errorCode: string): Promise<void> {
  await writeFailureEvidence(output, candidateSha, "environment-failure", 50, stage, errorCode);
}

export async function writeFailureEvidence(output: string, candidateSha: string, category: "output-failure" | "environment-failure", exitCode: 40 | 50, stage: string, errorCode: string): Promise<void> {
