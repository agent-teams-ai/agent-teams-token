...
export function dockerRunArguments(paths: ContainerPaths): readonly string[] {
  const script = [
    ...
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc /tools/forge build --skip test --skip script --use /tools/solc src/features/token-genesis/AGTMAIToken.sol",
    "set +e",
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc slither . --foundry-ignore-compile --foundry-out-directory /work/out --foundry-build-info-directory /work/out/build-info --config-file /input/tooling/security/slither/slither.config.json --json /work/gate-output/slither.json",
    "slither_status=$?",
    "set -e",
