...
const securityPrelude = [
  ...
  "cp -R /input/contracts /work/contracts", "cd /work/contracts/evm", "printf '%s\\n' version-inventory > /work/gate-output/failure.stage",
  "/tools/forge --version > /work/gate-output/forge.version 2>&1", "/tools/solc --version > /work/gate-output/solc.version 2>&1",
  "test \"$(command -v forge)\" = /tools/forge", "test \"$(command -v solc)\" = /tools/solc",
...
export function dockerCreateArguments(paths: ContainerPaths): readonly string[] {
  return hardenedCreateArguments(paths, [
    ...
    "mapfile -t targets < /input/tooling/security/slither/targets.txt", "test \"${#targets[@]}\" -gt 0",
    "for target in \"${targets[@]}\"; do [[ \"$target\" =~ ^src/[A-Za-z0-9._/-]+[.]sol$ ]] && [[ \"$target\" != *..* ]] && [[ \"$target\" != -* ]]; done",
    "printf '%s\\n' compiler-build > /work/gate-output/failure.stage",
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc /tools/forge build --offline --no-auto-detect --skip test --skip script --use /tools/solc \"${targets[@]}\"",
...
export function dockerVulnerableFixtureCreateArguments(paths: ContainerPaths): readonly string[] {
  return hardenedCreateArguments(paths, [
    ...securityPrelude, "printf '%s\\n' compiler-build > /work/gate-output/failure.stage",
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc /tools/forge build --offline --no-auto-detect --skip test --skip script --use /tools/solc src/Vulnerable.sol",
    "printf '%s\\n' analysis-runtime > /work/gate-output/failure.stage", "set +e",
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc slither . --fail-pedantic --foundry-ignore-compile --foundry-out-directory /work/out --foundry-build-info-directory /work/out/build-info --config-file /input/tooling/security/slither/slither.config.json --json /work/gate-output/slither.json",
    "slither_status=$?", "set -e", "printf '%s\\n' \"$slither_status\" > /work/gate-output/slither.exit", "test -s /work/gate-output/slither.json",
    "printf '%s\\n' artifact-validation > /work/gate-output/failure.stage", "test \"$(find /work/out/build-info -maxdepth 1 -type f -name '*.json' | wc -l)\" = 1",
...
function hardenedCreateArguments(paths: ContainerPaths, script: string): readonly string[] {
  return ["create", "--platform", "linux/amd64", "--network", "none", "--read-only", "--user", CONTAINER_UID_GID,
    ...
    `PATH=/tools:${paths.imagePath}`, `PYTHONPATH=${paths.pythonPath}`, "FOUNDRY_DISABLE_NIGHTLY_WARNING=1", "/bin/bash", "-ceu", script];
}
