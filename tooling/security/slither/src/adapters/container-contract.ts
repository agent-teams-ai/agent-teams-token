...
export const CONTAINER_UID_GID = "65532:65532";
...
export function dockerRunArguments(paths: ContainerPaths): readonly string[] {
  const script = [
    ...
    "slither --version > /work/gate-output/slither.version 2>&1",
    "crytic-compile --version > /work/gate-output/crytic-compile.version 2>&1",
    "slither --list-detectors > /work/gate-output/detectors.txt 2>&1",
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc /tools/forge build --skip test --skip script --use /tools/solc",
    "set +e",
    "FOUNDRY_OUT=/work/out FOUNDRY_CACHE_PATH=/work/cache FOUNDRY_BUILD_INFO_PATH=/work/out/build-info SOLC=/tools/solc slither . --foundry-ignore-compile --config-file /input/tooling/security/slither/slither.config.json --json /work/gate-output/slither.json",
    "slither_status=$?",
    "set -e",
  ...
  return [
    ...
    "--tmpfs", "/work:rw,nosuid,nodev,size=768m,uid=65532,gid=65532,mode=0700",
    "--tmpfs", "/home/gate:rw,nosuid,nodev,noexec,size=16m,uid=65532,gid=65532,mode=0700",
