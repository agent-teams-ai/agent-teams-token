...
const creationInput = "0x60006000";
let suiteRoot = "";
let manifestSource = "";
let buildSource = "";
let artifactSource = "";
...
before(async () => {
  ...
  const compiler = join(repositoryRoot, "packages/contexts/supply/dist/features/genesis-manifest/composition/cli.js");
  const compiledRoot = join(suiteRoot, "compiled");
  await execute(process.execPath, [compiler, "compile-local", join(repositoryRoot, "config/genesis/local.fixture.yaml"), compiledRoot]);
  const digestDirectory = strip0x(APPROVED_LOCAL_FIXTURE_ARTIFACT_SHA256);
  manifestSource = join(compiledRoot, digestDirectory, "manifest.json");
  ...
  await execute("forge", ["build", "--out", forgeOut, "--build-info", "--build-info-path", forgeBuildInfo, "--cache-path", join(suiteRoot, "forge-cache")], {
...
async function makeCase(): Promise<CaseContext> {
  ...
  const input: VerificationInput = {
    ...
    toolVersions: { node: process.version, forge: "1.8.0", cast: "1.8.0", anvil: "1.8.0" }, reportOutputRoot: join(directory, "reports"), runId: "test-run",
  };
